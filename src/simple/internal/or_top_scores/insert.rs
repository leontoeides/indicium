...
impl<'a, K: Hash + Ord> OrTopScores<'a, K> {
    ...
    pub(crate) fn insert(
        ...
        // Check if the `OrTopScores` struct has reached its maximum capacity:
        if self.top.len() >= self.capacity {

            // If the `OrTopScores` is at capacity and the lowest top score (the
            ...
            // The `OrTopScores` struct has not reached its capacity, we may
