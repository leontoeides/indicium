        ...
        if let Some(strsim_metric) = &self.strsim_metric {
            match strsim_metric {
                StrsimMetric::DamerauLevenshtein => self
                    .strsim_autocomplete_global_damerau_levenshtein(index_range, user_keyword)
                    .collect(),

                StrsimMetric::Jaro => self
                    .strsim_autocomplete_global_jaro(index_range, user_keyword)
                    .collect(),

                StrsimMetric::JaroWinkler => self
                    .strsim_autocomplete_global_jaro_winkler(index_range, user_keyword)
                    .collect(),

                StrsimMetric::Levenshtein => self
                    .strsim_autocomplete_global_levenshtein(index_range, user_keyword)
                    .collect(),

                StrsimMetric::SorensenDice => self
                    .strsim_autocomplete_global_sorensen_dice(index_range, user_keyword)
                    .collect(),
            } // match
        } else {
