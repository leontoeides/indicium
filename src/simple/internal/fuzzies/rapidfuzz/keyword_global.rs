...
impl<K: Hash + Ord> crate::simple::search_index::SearchIndex<K> {
    ...
    ///   be examined.
    ///
    /// * This method differs from `rapidfuzz_keyword_global_comparator` in that
    ...
    /// #
    /// #
    /// let keyword_substitution = search_index.rapidfuzz_keyword_global("Harry");
    ///
    /// assert_eq!(
    ...
    pub fn rapidfuzz_keyword_global(
        ...
        // caller:
        let rapidfuzz_metric = self.rapidfuzz_metric.as_ref()?;
        ...
        let global_keyword: Option<&kstring::KString> = match rapidfuzz_metric {
            RapidfuzzMetric::DamerauLevenshtein =>
                self.rapidfuzz_keyword_global_comparator::<DamerauLevenshtein>(
                    &index_range,
                    &user_keyword
                ),

            RapidfuzzMetric::Hamming =>
                self.rapidfuzz_keyword_global_comparator::<Hamming>(
                    &index_range,
                    &user_keyword
                ),

            RapidfuzzMetric::Indel =>
                self.rapidfuzz_keyword_global_comparator::<Indel>(
                    &index_range,
                    &user_keyword
                ),

            RapidfuzzMetric::Jaro =>
                self.rapidfuzz_keyword_global_comparator::<Jaro>(
                    &index_range,
                    &user_keyword
                ),

            RapidfuzzMetric::JaroWinkler =>
                self.rapidfuzz_keyword_global_comparator::<JaroWinkler>(
                    &index_range,
                    &user_keyword
                ),

            RapidfuzzMetric::LcsSeq =>
                self.rapidfuzz_keyword_global_comparator::<LcsSeq>(
                    &index_range,
                    &user_keyword
                ),

            RapidfuzzMetric::Levenshtein =>
                self.rapidfuzz_keyword_global_comparator::<Levenshtein>(
                    &index_range,
                    &user_keyword
                ),

            RapidfuzzMetric::Osa =>
                self.rapidfuzz_keyword_global_comparator::<Osa>(
                    &index_range,
                    &user_keyword
                ),

            RapidfuzzMetric::Postfix =>
                self.rapidfuzz_keyword_global_comparator::<Postfix>(
                    &index_range,
                    &user_keyword
                ),

            RapidfuzzMetric::Prefix =>
                self.rapidfuzz_keyword_global_comparator::<Prefix>(
                    &index_range,
                    &user_keyword
                ),
        }; // match
