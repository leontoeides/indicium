...
impl<K: Hash + Ord> crate::simple::search_index::SearchIndex<K> {
    ...
    ///
    /// * This method differs from `strsim_autocomplete_context` in that this
    ///   is a generic method. This method will be monomorphized for each
    ///   `strsim` string similarity metric (`DamerauLevenshtein`, `Jaro`, etc.)
    ///
    ///   `strsim_autocomplete_context` will call these monomorphized methods
    ///   using dynamic-dispatch, based on the search index's string similarity
    ///   metric settings.
    #[inline(always)]
    pub(crate) fn strsim_autocomplete_context_comparator<M>(
        &self,
        index_range: &str,
        key_set: &BTreeSet<&K>,
        user_keyword: &str,
    ) -> impl Iterator<Item = (&KString, &BTreeSet<K>)>
    where M: crate::simple::internal::fuzzies::strsim::Metric {
        ...
        self.b_tree_map
            ...
            .for_each(|(index_keyword, index_keys)| {
                ...
                let score = M::similarity(index_keyword, user_keyword);
