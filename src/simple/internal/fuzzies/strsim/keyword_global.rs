...
impl<K: Hash + Ord> crate::simple::search_index::SearchIndex<K> {
    ...
    ///   be examined.
    ///
    /// * This method differs from `strsim_keyword_global_comparator` in that
    ...
    /// #
    /// #
    /// let keyword_substitution = search_index.strsim_keyword_global("Harry");
    ///
    /// assert_eq!(
    ...
    pub(crate) fn strsim_keyword_global(
        ...
        // caller:
        let strsim_metric = self.strsim_metric.as_ref()?;
        ...
        let global_keyword: Option<&kstring::KString> = match strsim_metric {
            StrsimMetric::Jaro =>
                self.strsim_keyword_global_comparator::<Jaro>(
                    &index_range,
                    &user_keyword
                ),

            StrsimMetric::JaroWinkler =>
                self.strsim_keyword_global_comparator::<JaroWinkler>(
                    &index_range,
                    &user_keyword
                ),

            StrsimMetric::DamerauLevenshtein =>
                self.strsim_keyword_global_comparator::<DamerauLevenshtein>(
                    &index_range,
                    &user_keyword
                ),

            StrsimMetric::Levenshtein =>
                self.strsim_keyword_global_comparator::<Levenshtein>(
                    &index_range,
                    &user_keyword
                ),

            StrsimMetric::SorensenDice =>
                self.strsim_keyword_global_comparator::<SorensenDice>(
                    &index_range,
                    &user_keyword
                ),
        }; // match
