...
impl<K: std::hash::Hash + Ord> crate::simple::search_index::SearchIndex<K> {
    ...
    ///   lowercase.
    ///
    /// * This method differs from `eddie_global_metric` in that this method
    ///   will perform some common setup, and dynamically dispatch to the
    ///   generic method indicated by the chosen string similarity metric
    ///   (`DamerauLevenshtein`, `Jaro`, etc.)
    ///
    /// # Basic Usage
    ///
    /// ```ignore
    ...
    pub(crate) fn eddie_global<'s>(
        ...
        match self.index_range(last_keyword) {
            ...
            // `SearchIndex`:
            Some(index_range) => match self.eddie_metric.as_ref() {
                Some(EddieMetric::Levenshtein) => self
                    .eddie_global_metric::<Levenshtein>(
                        preceding_keywords,
                        last_keyword,
                        &index_range,
                        &mut top_scores
                ...
                Some(EddieMetric::DamerauLevenshtein) => self
                    .eddie_global_metric::<DamerauLevenshtein>(
                        preceding_keywords,
                        last_keyword,
                        &index_range,
                        &mut top_scores
                ...
                Some(EddieMetric::Jaro) => self
                    .eddie_global_metric::<Jaro>(
                        preceding_keywords,
                        last_keyword,
                        &index_range,
                        &mut top_scores
                ...
                Some(EddieMetric::JaroWinkler) => self
                    .eddie_global_metric::<JaroWinkler>(
                        preceding_keywords,
                        last_keyword,
                        &index_range,
                        &mut top_scores
