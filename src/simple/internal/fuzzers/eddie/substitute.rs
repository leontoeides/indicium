...
impl<K: std::hash::Hash + Ord> crate::simple::search_index::SearchIndex<K> {
    ...
    /// # Notes
    ///
    /// * This method differs from `eddie_substitute_comparator` in that
    ...
    /// #
    /// #
    /// let keyword_substitution = search_index.eddie_substitute("Harry");
    ///
    /// assert_eq!(
        ...
        let best_match: Option<&str> = match self.index_range(&user_keyword) {
            ...
            Some(index_range) => match self.eddie_metric.as_ref() {
                Some(EddieMetric::Levenshtein) =>
                    self.eddie_substitute_comparator::<Levenshtein>(
                        &user_keyword,
                        &index_range,
                    ),

                Some(EddieMetric::DamerauLevenshtein) =>
                    self.eddie_substitute_comparator::<DamerauLevenshtein>(
                        &user_keyword,
                        &index_range,
                    ),

                Some(EddieMetric::Jaro) =>
                    self.eddie_substitute_comparator::<Jaro>(
                        &user_keyword,
                        &index_range,
                    ),

                Some(EddieMetric::JaroWinkler) =>
                    self.eddie_substitute_comparator::<JaroWinkler>(
                        &user_keyword,
                        &index_range,
                    ),
