...
impl<K: std::hash::Hash + Ord> crate::simple::search_index::SearchIndex<K> {
    ...
    /// # Notes
    ///
    /// * This method differs from `eddie_keyword_comparator` in that this
        ...
        match self.index_range(user_keyword) {
            ...
            Some(index_range) => match self.eddie_metric.as_ref() {
                Some(EddieMetric::Levenshtein) => self
                    .eddie_keyword_comparator::<Levenshtein>(
                        user_keyword,
                        &index_range,
                        &mut top_scores
                    ),
                ...
                Some(EddieMetric::DamerauLevenshtein) => self
                    .eddie_keyword_comparator::<DamerauLevenshtein>(
                        user_keyword,
                        &index_range,
                        &mut top_scores
                    ),
                ...
                Some(EddieMetric::JaroWinkler) => self
                    .eddie_keyword_comparator::<JaroWinkler>(
                        user_keyword,
                        &index_range,
                        &mut top_scores
                    ),
